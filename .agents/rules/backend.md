...
5. **テーブル駆動テスト（Table-Driven Tests）の採用**
   - Go のユニットテストは、保守性と拡張性を高めるために、原則としてテーブル駆動テスト（`tests` スライスと `t.Run` を用いた構造）として記述してください（例: `internal/repository/cache_fs_test.go` などの記述形式に準拠します）。単一のケースしか存在しない単純なテスト関数であっても、将来的なケース追加を見据えてテーブル型で実装します。
