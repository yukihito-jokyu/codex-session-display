...
| フィールド     | 型                             | 説明                                             |
| -------------- | ------------------------------ | ------------------------------------------------ |
| Index          | 整数                           | 通常ターンは0始まり、疑似ターンは-1             |
| TurnID         | 文字列                         | 通常ターンの識別子。疑似ターンでは空文字列       |
...
※Wailsのローカルファイル埋め込み（`embed.FS`）およびWebview環境での動作互換性を維持するため、React Routerには `HashRouter`（URL末尾の `#` を基点とするハッシュルーティング方式）を採用する。これにより、アプリのリロードや画面の直接起動時におけるファイルパスの不整合（404エラー）を防止する。
...
React Flow は初期表示時から viewport 内の要素のみを DOM に描画する。未測定ノードも初期 `fitView` と可視判定に含め、viewport 外ノードの初回強制描画を避けるため、カスタムノードの既知の最小寸法（幅320px、高さ80px）と各ハンドルの初期位置を渡す。カスタムノードは memo 化し、ノード選択や下部パネルの状態更新によって内容が変わらないノードを再レンダリングしない。token badge のクリックコールバックはノードごとに再生成せず、安定した共通コールバックへノードIDを渡す。

### 3.3 Wailsバインディング

Wailsの起動・ビルドプロセス（`wails dev` または `wails build`）により、Goの `App` メソッドに対応するTypeScript型付き関数が `frontend/wailsjs/go` 配下に自動生成される。フロントエンドは生成されたバインディングを通じてバックエンドと通信する。
...
`wails dev` 実行時または `wails build` 実行時に、Goの構造体・メソッドからTypeScriptの型定義と呼び出し関数が自動的に `frontend/wailsjs/go` 配下に生成される。生成されたファイルはフロントエンドからインポートして使用する。

#### 3.3.2 エラーハンドリング

バインディング関数はGo側のエラーをそのまま返す。Go側が `AppError` を返した場合、フロントエンドでは `code` フィールドでエラー種別を判定し、対応するメッセージを表示する（§6.2参照）。
...
├──────────────────┬──────────────────────────┬─────────────┤
│ Conversation     │ React Flow Canvas        │ RightPanel  │
│ Timeline         │ (ズーム・パン可能)       │             │
│                  ├──────────────────────────┤             │
│                  │ Bottom Panel             │             │
└──────────────────┴──────────────────────────┴─────────────┘
...
- `body` とラベル付き `details` を連結した全文が600文字を超える場合、タイムラインDOMには先頭600文字と省略記号だけを描画する。
- 省略時だけ「全文を表示」ボタンを表示し、`onShowFullText(item)` で対象項目を画面状態へ通知する。
- `useSessionDetail` は通知された項目から一時的な `FlowNode` を構築し、既存のノード詳細モードでBottomPanelへ全文を表示する。バックエンドDTOとキャッシュスキーマは変更しない。
- タイムライン由来の詳細を表示中もキャンバスノードを直接選択でき、選択後は通常のノード詳細へ切り替える。
- 600文字以下の内容は従来どおり本文と `details` を表示し、「全文を表示」は表示しない。
- 展開状態はコンポーネント内で管理し、永続化しない。
- 推論、ツール・コマンド、Web、MCP、instructions、system、参照情報を会話と同じJSONL時間軸へ配置する。
- 検索入力は `body` と `details[].value` を大文字・小文字を区別せず照合する。
- 種別フィルターは初期状態ですべて選択し、`kind` を次の5群へ分類する。
  - 会話: `conversation`
  - 推論: `reasoning`
  - ツール・コマンド: `tool`, `web`, `mcp`
  - 参照情報: `instructions`, `reference`
  - システムイベント: `system` および未知の種別
- 「トークン計測あり」は `token_count_count > 0` の項目だけを表示する。
- 検索、種別、トークン条件はANDで適用し、一致項目を持つターンだけを表示する。
- 絞り込み後も元のターン番号、疑似ターン表示、項目順を維持する。
- 一致項目がない場合は「条件に一致するタイムライン項目はありません」と表示する。
- フィルター状態はコンポーネント内で管理し、永続化しない。
...
- `fullText` がある場合は検索入力を表示し、大文字・小文字を区別せず全一致箇所を強調する。検索語入力中は一致件数を表示し、不一致時は0件と表示する
- 表示対象ノードが切り替わった場合は検索状態を初期化する
...
| message    | 文字列 | ユーザー向けエラーメッセージ（日本語） |
...
| code               | 条件                                                     |
| ------------------ | -------------------------------------------------------- |
| SESSION_NOT_FOUND  | 指定IDのセッションファイルが存在しない                   |
| FILE_TOO_LARGE     | ファイルサイズが50MBを超過                               |
| UNSUPPORTED_FORMAT | 非対応のセッションファイル形式（Codex CLI v0.121.0未満） |
| PARSE_ERROR        | JSONLの解析に失敗                                        |
| FILE_READ_ERROR    | ファイル読み込み失敗                                     |
| INTERNAL_ERROR     | 内部エラー                                               |

### 4.4 OpenLogDirectory()

**概要:** アプリケーションログが格納されているディレクトリをOS標準のファイルマネージャー（Mac: Finder, Windows: Explorer）で開く。

**Goシグネチャ:** `func (a *App) OpenLogDirectory() error`

**引数:** なし

**戻り値:** なし（非同期でOSのプロセスを起動する）

**エラー:**

| code           | 条件                                                   |
| -------------- | ------------------------------------------------------ |
| INTERNAL_ERROR | ログディレクトリの特定失敗、またはOSコマンドの起動失敗 |

### 4.5 GetLogFilePath()

**概要:** アプリケーションログファイル `app.log` の絶対パスを返す。フロントエンドの「ログパスをコピー」導線から利用する。

**Goシグネチャ:** `func (a *App) GetLogFilePath() (string, error)`
...
| code           | 条件                           |
| -------------- | ------------------------------ |
| INTERNAL_ERROR | ログファイルパスの特定に失敗   |

### 4.6 ExportStatsImage(id)（保留）
...
入力: ~/.codex/sessions/YYYY/MM/DD/rollout-*.jsonl
  │
  ├─ Step 1: ファイルチェック
  │   ├─ ファイル存在確認 → SESSION_NOT_FOUND
  │   ├─ ファイルサイズ > 50MB → FILE_TOO_LARGE
  │   └─ ファイルサイズ == 0 → PARSE_ERROR ("セッションファイルが空です")
  │
  ├─ Step 2: 行ごとパース
...
IPCエラーは `AppError` 構造体（§4.1参照）で返す。

| エラー               | code               | メッセージ                                                                 |
| -------------------- | ------------------ | -------------------------------------------------------------------------- |
| セッション未検出     | SESSION_NOT_FOUND  | 指定されたセッションが見つかりません                                       |
| ファイルサイズ超過   | FILE_TOO_LARGE     | ファイルサイズが上限（50MB）を超えています                                 |
| 非対応形式           | UNSUPPORTED_FORMAT | このセッションファイルの形式には対応していません（Codex CLI v0.121.0未満） |
| パース失敗           | PARSE_ERROR        | セッションファイルの解析に失敗しました                                     |
| ファイル読み込み失敗 | FILE_READ_ERROR    | セッションファイルの読み込みに失敗しました                                 |
| 画像生成失敗         | IMAGE_GEN_ERROR    | 画像の生成に失敗しました                                                   |
| 内部エラー           | INTERNAL_ERROR     | 内部エラーが発生しました                                                   |
...
- `wails dev` コマンドで開発モードを起動
- Goバックエンドのホットリロード + フロントエンドのViteホットリロードが同時に動作
- デスクトップウィンドウが自動で開く

### 8.2 プロダクション構成

- `wails build` コマンドでビルド
- フロントエンドのビルド成果物がGoバイナリに埋め込まれる
- 単一のネイティブ実行可能ファイル（`.app` / `.exe`）を生成
- 配布先でランタイム不要
...
| 中央配置 | 有効                    | |
| リサイズ | 可                      | |
| タイトルバー | 非表示 (TitleBarHidden) | OS標準タイトルバーを非表示とし、React側でカスタムタイトルバーを実装。 |
| ウィンドウ半透明 | 有効 (Translucent) | 背景にmacOS: Vibrancy、Windows: Mica効果（BackdropType: Mica）を適用。 |
| ドラッグ可能領域 | `--wails-draggable: drag` | フロントエンドのカスタムヘッダーにCSSカスタムプロパティを適用し、ウィンドウのドラッグ移動を可能にする。 |

### 8.4 シングルインスタンス制御と引数転送

多重起動の防止およびコマンドライン/ファイル関連付けからのパス受け渡しのため、以下の設計を行う。

#### 8.4.1 TCPソケットによる起動判定と引数送信

1. **リスナー取得による判定**:
   `main.go` の起動直後に、`localhost:58385`（固定ポート）で TCP リスナーの取得を試みる。
   ```go
   listener, err := net.Listen("tcp", "127.0.0.1:58385")
   ```
2. **メインインスタンス（成功時）の処理**:
   ポート取得成功時は、リスナーから接続を受け入れるゴルーチンをバックグラウンドで開始し、`wails.Run()` を実行する。
3. **二重起動インスタンス（失敗時）の処理**:
   ポート取得失敗時は、既存のインスタンスに対してTCPクライアントで接続し、`os.Args` の第1引数（セッションファイルパス、存在する場合）を文字列として送信した直後に終了（`os.Exit(0)`）する。

#### 8.4.2 受信イベントのフロントエンド連携

1. **引数受信と最前面表示**:
   メインインスタンスのバックグラウンドリスナーが接続を検知すると、送信されたファイルパスを読み取る。
   パスを読み取ったメインインスタンスは、以下のWailsランタイム処理を実行する。
   - `runtime.WindowShow(ctx)` により、ウィンドウを隠し状態や最小化状態から最前面に復帰・表示させる。
   - `runtime.EventsEmit(ctx, "open-session-file", filePath)` により、フロントエンドへパスを通知する。
2. **フロントエンド（React）の処理**:
   React側のエントリポイント（例: `App.tsx` または `index.tsx`）で `window.runtime.EventsOn("open-session-file", (path: string) => { ... })` をリッスンする。
   イベント受信時、`/sessions/:id` への画面遷移をトリガーし、バックエンドの `GetSessionDetail` を呼び出して該当セッションファイルを表示する。
...
##### TC-107: タイムラインの検索と絞り込み
- **前提**: 会話、推論、ツール・コマンド、参照情報、システムイベントと、計測済み・未計測の項目を含むセッション詳細画面が表示されている。
- **操作**: 本文または要約を検索し、種別チェックボックスと「トークン計測あり」を切り替える。
- **検証**:
  - 初期状態では5種別がすべて選択され、全項目が表示されること。
  - 本文と推論要約への検索が一致項目だけを表示すること。
  - 5種別を個別に非表示・再表示できること。
  - 「トークン計測あり」で `token_count_count > 0` の項目だけが表示されること。
  - 複数条件がANDで適用され、元のターン番号と項目順が維持されること。
  - 条件に一致しない場合は空状態が表示されること。

##### TC-108: 長いタイムライン内容の全文表示と検索
- **前提**: 600文字を超えるコマンド出力と短い参照情報を含むセッション詳細画面が表示されている。
- **操作**: 長い項目を展開して「全文を表示」を押し、BottomPanelの検索入力へ検索語を入力する。
- **検証**:
  - 長い項目はタイムライン上で先頭600文字に省略され、末尾の内容がDOMへ描画されないこと。
  - 「全文を表示」でBottomPanelに欠落のない全文が表示されること。
...
*   **`DEBUG`（デバッグ情報）**: 開発・トラブルシューティング用の詳細ログ。JSONLパースの各行の処理進捗、キャッシュ処理の詳細、内部レイアウト計算のステップなど。※通常は出力されず、デバッグモード時のみ有効化する。
*   **`INFO`（通常動作の記録）**: アプリケーションの正常な動作を示すイベント。アプリ起動/終了、JSONLパースの開始と成功（処理行数や所要時間、ノード数等のサマリー）、キャッシュの読み込み成功など。
*   **`WARN`（回復可能な問題 / データの不整合）**: 処理自体はフォールバック処理等によって継続可能だが、入力データに不整合や軽微な異常がある場合。1行パース失敗による該当行のスキップ、`call_id` の不整合によるインデックスベースの紐付け（フォールバック）発生、`turn_context` 重複による上書き、`session_meta` 欠損時のファイル名からのメタデータ推測など。
*   **`ERROR`（回復不可能な問題 / 主要機能の失敗）**: 処理の継続が不可能な場合、あるいは主要機能（キャッシュ等）が完全に失敗した場合。JSONLファイルの読み込みエラーによるパース中断、パース不正レコード率が50%を超えたことによる処理中断、キャッシュファイル（`~/.codex-display`）の書き込み/読み込み失敗など。
...
*   **構造化項目**:
    ...
    *   `line_preview` (string): 失敗した行の**先頭100文字**（巨大な行データによるログサイズ肥大化や、トークン等の秘匿情報流出を防ぐため、先頭100文字のプレビューに制限する）

#### 10.3.2 `call_id` 不整合によるインデックスベース紐付け発生時
...
#### 10.3.3 `turn_context` 重複による上書き発生時
...
#### 10.3.4 `session_meta` 欠損によるファイル名からの推測時
...
### 10.5 ログファイルの自動ローテーション

ログファイルの無制限な肥大化を防ぐため、起動時に自動ローテーションを実行する。

1. **サイズ確認**:
   起動の初期フェーズ（ロガー初期化前）で、`~/.codex-display/logs/app.log` のサイズを取得する。
2. **判定としきい値**:
   サイズが **10MB**（10,485,760 バイト）以上の場合、ローテーションをトリガーする。
3. **退避処理**:
   - `~/.codex-display/logs/app.log.1` が既に存在する場合は削除する。
   - `~/.codex-display/logs/app.log` を `~/.codex-display/logs/app.log.1` へリネームする。
4. **新規作成**:
   新しく空の `app.log` を作成し、これに対してロガーのファイルストリームを初期化する。

---

## 11. エラー監視・クラッシュレポート設計

### 11.1 外部監視サービスの不使用方針
ローカル完結の機密保持要件（§1.4, §3.1）を順守するため、SentryやFirebase Crashlyticsなどの外部へのデータ送信を伴う自動エラー監視サービスは一切使用しない。詳細は [ADR 0022](file:///Users/yukihito/Documents/github_projects/codex-session-display/docs/adr/0022-error-monitoring-and-crash-reporting-strategy.md) を参照。

### 11.2 代替となるローカルデバッグ・支援設計
自動エラー送信を行わない代替として、以下の仕組みを設計・実装する。

#### 11.2.1 フロントエンドエラー表示とログ導線
*   **エラー発生時のUI遷移**: バックエンドから `AppError`（`INTERNAL_ERROR`, `PARSE_ERROR` など）が返された際、またはフロントエンド自体の実行例外発生時、エラーメッセージとともに「再試行」ボタンを持つエラー用UIを表示する。
*   **「ログ保存先パスのコピー」機能**:
    *   エラー画面および設定画面等に配置する。
    *   クリック時に、ローカルログファイル（`~/.codex-display/logs/app.log`）の絶対パス（ユーザーのホームディレクトリを展開したもの）をクリップボードにコピーする。
*   **「ログフォルダを開く」機能**:
    *   同じくエラー画面等に配置する。
    *   クリック時に、新規追加したバックエンドIPCの `OpenLogDirectory()` メソッドを呼び出し、OS標準のファイルマネージャー（Mac: Finder, Windows: Explorer）で `~/.codex-display/logs/` ディレクトリを開く。
