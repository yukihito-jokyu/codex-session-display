...
`wails generate module` により、Goの `App` メソッドに対応するTypeScript型付き関数が自動生成される。フロントエンドは生成されたバインディングを通じてバックエンドと通信する。
...
`wails generate module` を実行すると、Goの構造体・メソッドからTypeScriptの型定義と呼び出し関数が自動生成される。生成されたファイルはインポートして使用する。
