...
処理フロー:
  ...
  2. .jsonl ファイルを検出
  3. ファイルパスからセッションIDを抽出（UUIDパターン）
  4. ファイルパスからタイムスタンプを推測（YYYY-MM-DDTHH-MM-SS）
  5. ファイルサイズを取得
  6. ファイル更新日時を取得
  7. キャッシュを確認:
...
JSONLファイルの最終更新日時 > キャッシュファイルの最終更新日時 の場合、再パースが必要。
...
| シナリオ                   | 表示方法                                         |
| -------------------------- | ------------------------------------------------ |
| API呼び出し失敗            | 画面中央にエラーメッセージを表示                 |
| セッション一覧が空         | 「セッションが見つかりません」の空状態メッセージ |
| セッション詳細のパース失敗 | 「セッションファイルの解析に失敗しました」を表示 |
| ネットワークエラー         | 「サーバーに接続できません」を表示               |
| ローディング中             | スピナーまたはスケルトンローディングを表示       |
