...
- 同じ統計項目の連続操作でも代表ノードへの移動を毎回実行する。右側統計パネル自体の
  スクロール位置は変更しない
...
├──────────────────┬──────────────────────────────┬────────────────────┤
│ タイムライン     │ React Flowキャンバス          │ 統計・トークン分析 │
│ - ターン区切り   │ （ハーネス表示）              │                    │
│ - 会話/AI行動    ├──────────────────────────────┤                    │
...
- タイムライン: 左側。会話、推論、ツール・コマンド、Web、MCP、instructions、system、参照情報をJSONL順で表示する
  - 初期幅は画面幅の30%、最小幅は320px、最大幅は画面幅の50%とする
  - キャンバスとの境界をポインターでドラッグ、またはフォーカスして左右矢印キーで操作し、表示幅を変更できる
  - 変更した幅は端末内へ保存し、別セッションの表示および次回起動時に復元する
  - 画面幅が縮小した場合は、保存済みの幅を現在の最大幅へ再調整する
  - User/AI会話は常時展開し、会話以外は種類名と統合レコード件数を表示して初期状態では折りたたむ
  - 展開時は推論本文・要約、ツール名・引数・結果、コマンド・出力、検索クエリ・閲覧対象、MCPサーバー・引数・結果、instructions本文、参照パスを確認できる
  - 本文と詳細を連結した内容が600文字を超える場合は先頭600文字だけを表示し、「全文を表示」から下部詳細パネルで欠落なく閲覧できる
  - 600文字以下の内容は省略せず、「全文を表示」を表示しない
  - 本文と要約を大文字・小文字を区別せず検索できる
  - 会話、推論、ツール・コマンド、参照情報、システムイベントの5種別を個別に表示・非表示にでき、初期状態では全種別を表示する
  - `token_count` の紐付けがある表示単位だけに限定でき、検索・種別・トークン条件はANDで適用する
  - 絞り込み後も元のターン番号、疑似ターン表示、項目順を維持し、一致項目がない場合は空状態を表示する
  - 通常ターンは所要時間とターン消費トークンを表示し、疑似ターンは「ターン外イベント」と表示する
  - 同一ターン内の同一ロール・本文・タイムスタンプの発言は1件へ統合する
  - 各表示単位に紐付く全 `last_token_usage` を合算し、紐付け件数と最新 `total_token_usage` を表示する。紐付けがない場合は「計測なし」と表示する
- タイムライン、React Flowノード、token_countバッジ、TOKEN COUNT LOG行、
  Last Token Consumption per Indexの点は同じ表示単位を共通選択する
  - 選択中のタイムライン項目、代表ノード、関連する全統計行・全系列の点を同じアクセントで強調する
  - タイムライン以外からの選択時は対応項目へ自動スクロールし、ノード以外からの選択時は代表ノードへズームする
  - 右側統計パネルは自動スクロールせず、対応要素の強調だけを更新する
  - 同じ表示単位に複数のtoken_countがある場合は全件を同時に強調する
  - タイムライン項目の再選択、キャンバス背景、下部詳細パネルの閉じる操作で全領域の選択を解除する
...
- ノード詳細パネル: 下部。ノード/token_countバッジまたはタイムラインの「全文を表示」操作で展開し、全文を大文字・小文字を区別せず検索して一致箇所と件数を確認できる
