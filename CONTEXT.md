...
**疑似ターン（Pseudo-Turn）**:
通常のターン（`task_started` 〜 `task_complete`）の枠外（セッション開始直後、ターン間、セッション終了後）に出現する「ターン外レコード」や、対応する `task_started` が欠落した「孤立した終了イベント」をグループ化し、内部的にターンと同様に扱うための仮想的な単位。
...
`agent_reasoning` がなく、`response_item(type=reasoning)` の `summary` も空で本文を表示できない場合は、「（暗号化済み・表示不可）」として扱う。この表示不能な reasoning が UI 上で連続する場合は、ノイズを抑えるため1ノードに集約し、件数を `×N` で表示する。`token_count` など表示ノードを生成しないレコードは連続性を分断しない。

ターン内の Reasoning、Tool Batch、Agent Message などの動的ノードは、JSONL レコードから表示単位を構築し、代表レコードの出現順でハーネスへ配置する。
